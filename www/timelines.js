function load_timelines() {
  $.getJSON('/time_series.json',function(data) {
    ...
    options.grid.markings = [ { yaxis: { from: 33.26436, to: 33.26436}, color: "#ff0000"}, { yaxis: { from: 29.63794, to: 29.63794}, color: "#0000ff"} ]
    options.yaxis.min = 29;
    options.yaxis.max = 34;
    $.plot('#psnrhvs_graph',[data[1].sort()],options);
    options.grid.markings = [ { yaxis: { from: 12.05351, to: 12.05351}, color: "#ff0000"}, { yaxis: { from: 9.42931, to: 9.42931}, color: "#0000ff"} ]
    options.yaxis.min = 9;
    options.yaxis.max = 13;
    $.plot('#ssim_graph',[data[2].sort()],options);
    options.grid.markings = [ { yaxis: { from: 14.71761, to: 14.71761}, color: "#ff0000"}, { yaxis: { from: 11.56938, to: 11.56938}, color: "#0000ff"} ] 
