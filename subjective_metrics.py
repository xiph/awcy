...
videos = ['subjective-wip/MINECRAFT_420_300f.y4m','subjective-wip/Netflix_Crosswalk_1920x1080_30fps_8bit_420_300f.y4m','subjective-wip/Netflix_TunnelFlag_1920x1080_30fps_8bit_420_300f.y4m','subjective-wip/sintel_trailer_cut1.y4m','subjective-wip/vidyo1_720p_30fps_300f.y4m']
...
for k,v in votes_per_user.items():
    for video in videos:
        cur = db.execute("SELECT * from votes WHERE decoders = ? AND videos LIKE ? AND voter = ?;",(decoders,video+'%',k))
...
print(decoders)
print("%40.40s   %5s %5s %5s %5s %.5s" % ("Video",labels[0],"Tie",labels[1],"Total","P-value"))
...
def get_non_duplicate_votes(db, video):
    cur = db.execute("SELECT * from votes WHERE decoders = ? AND videos LIKE ?;",(decoders,video+'%'))
    vote_by_user = {}
